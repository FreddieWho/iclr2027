...
> **2026-09-25 本轮非 M2 补算的审查与修复已收口。** 当前入口：`reports/submission_audit_20260925/`。
> 已证实旧 M2 微调分支没有 backbone 梯度、冻结分支更新了 BatchNorm，且初始化早于设种子；旧“精确归因／排除容量”结论撤回。修复后完整 16 格使用用户提供的现有 GPU 补算。route2 曝光匹配的 3 个静态基线已完成并逐文件回收校验；21 模型单编辑重叠排除敏感性也已完成。M2 补算按用户最新指示后置，当前不纳入新科学结论。
> 下文为此前各轮记录；其中“全部闭环”不能替代本轮验收。当前任务是证据与代码准备，不宣称论文已完成投稿。
