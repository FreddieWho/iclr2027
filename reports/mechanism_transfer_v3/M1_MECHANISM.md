...
- Source G8 bounded search (`source_search.json`, bank SHA `15f5bf18`, 4096 scenes): nearest opposite-label pair distance 0.0471; 3000-step local search reached 0.0335; no exact merge. Equal-label control is closer (0.0213). This is the best observed distance and an upper bound on the global minimum, not a positive lower bound or a completeness proof.
...
## Claim level
- Supported: the whole-orbit reference outperforms the sorted, unsorted, and raw arms under clean supervision on this bank. The observed flip-side contrasts do not establish either a consistent gain or noninferiority to the sorted arm. Invariance alone does not determine performance in the tested model set.
