...
### 3.1 胜算评估（纯科学维度；时间/行政因素按用户指示剥离）

以下只评估科学内容本身在 ICLR 主赛道的竞争力（基准接收率 27.4%）：

| 科学状态 | 估计胜算 | 关键条件 |
|---|---|---|
| 现有证据原样成文 | 30–40% | 新颖性清晰、证据链完整；但第四部分 A/B/C 三个科学缺口不补，会被理论意识强的审稿人压住 |
| 补齐第四部分的关键项（A/B/C＋D 中两项对照） | 45–55% | 近似适用域量化＋构造性平凡化改写＋因果分层＋两个对照 |

判断依据：新颖性检索无直接撞车（2.2）；体裁有主赛道先例（2.3）；外部确认是真实优势。主要扣分项全部在科学缺口本身（见第四部分），不在规模或领域。
...
## 第四部分：纯科学性深度评估（2026-09-05 增补，应用户要求剥离时间因素）

本部分只问一个问题：**证据和主张在科学上站不站得住**。核验了新发现的三个关键事实：
`z_mode` 平移测试中 centered 视图在平移后**重新定心**（`centered_input_recomputed_after_translation`）；干预 ε=0.25 为冻结单点，无剂量扫掠；kNN 图 k=4 无敏感性检验；无未训练模型对照。

### A.【最重要的理论张力】局部近似何时是发现、何时是定义

论文核心等式 1−cos ≈ ½δᵀG_fδ 是泰勒展开：当 ε→0 时"几何预测响应"趋于定义性同义反复。
真正的科学内容只在三层：
1. **有限 ε=0.25 处局部近似的保持度**——这是可测量、且理论自身给出可证伪预测
   （预测质量应随 ε 单调退化）。当前 ε 单点冻结，此曲线缺失。
   **补上 ε-sweep（纯推理＋Jacobian，无训练、无协议冲突）能把"同义反复"质疑转化为论文的一个实证章节。**
2. **排序预测**：几何预测的是 anchor-vs-control 的**相对响应排序**，不是绝对值——
   主张措辞必须精确到"relative response ranking under the frozen protocol"。
3. **迁移性**：同一几何→响应关系跨模型、跨比赛、跨 provider 保持（已有证据），
   这是超出定义层的实质内容。

### B.【科学诚实问题】z_mode 平移近零是构造保证，不是学到的性质

dual-channel 与 centered 模型的 intrinsic 视图在平移后重新定心，因此 z_mode 响应≈0
**由输入构造保证**，与模型无关。现状把它与任务指标并列呈现，审稿人一眼看穿。
正确写法：
- 平移不变性表述为**设计保证＋健全性检查**（sanity check）；
- 实证亮点改为：context 通道在平移下仍可访问（accessibility 保持）、raw 单通道基线
  0.256 的响应（鱼与熊掌不可兼得的实证）、以及双通道各自担当的语义无泄漏
  （cross-readout）。
这不是措辞美化，是把一个"看似结果"还原为"设计＋检验"的诚实分层。

### C.【因果语言分层】现有证据支持到哪一层

| 层级 | 证据 | 允许的措辞 |
|---|---|---|
| 几何→响应排序 | 预测性（Spearman 0.85/0.63/逐场 0.42–0.60） | "predicts / anticipates"，不可用 causal |
| pooling 开关改变 block 组成与响应 | 干预性（P3-C5，matched capacity） | "response shaping via causal switch" |
| 2:1 修复 | 应表述为**对诊断的可证伪检验**：若诊断（context/intrinsic 混淆）为真，分通道应消除病理——8/8 同向支持 | "the repair validates the diagnosis"，不是"SOTA 方法" |

科学上最硬的框架是：**修复是诊断的验证实验，不是产品**。这同时回答"so what"。

### D.【缺失的对照，全部有独立科学理由而非讨好审稿人】

1. **未训练模型对照**：未训练网络也有光滑局部几何。若几何→响应在未训练模型上同样成立，
   则主张应收缩为"光滑映射的通用性质＋训练改变了几何的 support 依赖结构"；若不成立，
   则训练带来的几何结构是机制的一部分。两种结果都有信息量，不做则主张边界说不清。
2. **SSL 目标族对照**（SimSiam/BYOL 类，同数据）：检验 support 敏感病理是**目标通用**
   还是我们目标特有——这是现象学的核心问题，不是规模问题。
3. **kNN k 值敏感性**：谱特征依赖图构造（k=4 固定未测）；k 扫掠确认结论不随图参数翻转。
4. **简单预测基线**（原始坐标统计、CKA、attention rollout）：证明几何特征携带非平凡信息。
5. **support 大小伸缩**（1→k 节点）：检验 support 条件化的粒度依赖——
   "谁和谁一起动"的分辨率本身就是科学问题。

### E.【统计深度的如实呈现】

- 8/8 同向：符号检验 p=2⁻⁸≈0.004，可显式陈述为方向一致性证据。
- 但任务效应量小：zone F1 +0.005、pair +0.009、geometry +0.034（macro）。
  **幅度最大的是 centroid MAE 0.0365 vs 0.0554（−34%）**，应作为 headline 之一。
  小效应＋全同向的诚实表述优于夸大。
- 层级结构：场（n=8）是独立单位，seed（n=3）是同条件重复；match-level bootstrap CI 必须给。
- 最弱一场（132877 full=0.421）已在报告中如实记录——保留，审稿人尊重这种透明。

### F.【科学范围边界（必须在 Limitations 显式声明）】

- 单帧快照表示，**无时间维度**；1Hz 采样。时间编码器上的可迁移性未知。
- 单一 SSL 目标族、GAT/Phase-GAT 家族、~140k 参数——机制结论的范围即此家族。
- 干预是合成重分配，非真实战术变化；与真实战术语义的距离未测量。
- 两个 provider 都是足球；级别差异（职业 vs 大学）反而是分布外强度的正面卖点。

### G. 科学性提升清单（纯科学收益排序，与时间表无关）

1. ε-sweep＋局部近似失效曲线（A）——把一个潜在致命质疑变成论文亮点
2. 未训练对照（D1）——划清主张边界
3. SSL 目标族对照（D2）——回答现象通用性
4. z_mode 平移表述改写（B）——零成本，纯诚实性修正
5. 因果语言分层落地到摘要/正文（C）——零成本
6. k 敏感性＋简单预测基线＋support 伸缩（D3–D5）——稳健性补全
7. bootstrap CI＋符号检验＋效应量诚实表（E）

以上 1–3 需要新计算但全部可在开发集完成、不触碰任何已消耗的保留场/外部读取；
4/5 是写作动作；6/7 是低成本分析。
