...
This distinction connects two familiar concerns that are usually measured separately. Compositional-generalization benchmarks ask whether learned components can be recombined \citep{lake2018generalization, keysers2020measuring}, and conditional compositionality gaps isolate failures despite success on the component problems \citep{press2023measuring}. Model-update research asks whether a new predictor introduces errors on previously correct examples \citep{yan2021positive}. We bring these concerns together \textbf{within the same controlled instance}. The constituents are not an unrelated regression set: their correctness is precisely what makes a failed composition informative.
...
\tightlist
\item
  \textbf{A paired protocol for compositional repair.} We construct oracle-defined quartets, fix the baseline failure population, and separate full repair from error relocation through an exact accounting identity.
\item
  \textbf{Evidence that endpoint repair can be dominated by relocation.} On the coordinate task, 79--94\% of endpoint repairs fail to preserve both constituent successes. A factorial comparison distinguishes this outcome from the larger joint gains of geometric inputs with change supervision.
\item
  \textbf{A practical reporting criterion.} Joint accuracy, constituent coverage, conditional composition error, and paired repair flows provide complementary information. A matched visual comparison illustrates the criterion beyond direct coordinate inputs.
\end{enumerate}

The paper contributes a diagnostic and an empirical finding, rather than a general-purpose composition architecture. Conditional evaluation itself has substantial prior work; our focus is what changes when a purported repair is evaluated on the \emph{same} constituents that justified the original diagnosis.

\hypertarget{a-controlled-test-of-composition}{%
\section{A controlled test of composition}\label{a-controlled-test-of-composition}}

\hypertarget{scenes-edits-and-oracle-eligibility}{%
\subsection{Scenes, edits, and oracle eligibility}\label{scenes-edits-and-oracle-eligibility}}
...
\begin{figure}
\centering
\includegraphics[width=1\textwidth,height=\textheight]{figures/fig1_protocol.pdf}
\caption{\textbf{An endpoint correction has two distinct outcomes.} The diagram follows correctness on A, B, and AB. A baseline pattern of 110 becomes a full repair only at 111. Correcting AB while breaking A or B relocates the error. The diagram is a schematic, not an empirical example; the bits indicate correctness, not predicted classes.}
\end{figure}

\hypertarget{complementary-scores-on-a-fixed-quartet-bank}{%
\subsection{Complementary scores on a fixed quartet bank}\label{complementary-scores-on-a-fixed-quartet-bank}}
...
\hypertarget{repair-or-relocation}{%
\section{Repair or relocation?}\label{repair-or-relocation}}

\hypertarget{fixing-the-population-before-comparing-models}{%
\subsection{Fixing the population before comparing models}\label{fixing-the-population-before-comparing-models}}
...
\hypertarget{endpoint-success-can-conceal-constituent-failure}{%
\section{Endpoint success can conceal constituent failure}\label{endpoint-success-can-conceal-constituent-failure}}

\hypertarget{composition-fails-despite-correct-constituents}{%
\subsection{Composition fails despite correct constituents}\label{composition-fails-despite-correct-constituents}}

On the fresh holdout, the clean coordinate classifier misses \textbf{151 of 176 eligible compositions (85.8\%)}, conditional on both constituent predictions being correct. This is a failure rate within a precisely defined population, not an error rate over arbitrary edits or scenes. The 176 cases must not be interpreted as 176 independent parents; the preserved aggregate does not support a parent-cluster interval for this headline. Its role is to establish that the composition failure extends beyond the diagnostic bank.
...
The fixed-baseline decomposition makes the discrepancy explicit (Figure 2). Endpoint repair is 0.427, 0.425, and 0.482, whereas full repair is only 0.064, 0.025, and 0.100. Error relocation accounts for \textbf{85.1\%, 94.1\%, and 79.2\%} of corrected endpoints. The paired joint-accuracy differences are 0.013, 0.008, and 0.021; their parent-cluster intervals all include zero. The endpoint improvement is real, but most corrected endpoints do not retain the constituent successes that defined the original problem.

\begin{figure}
\centering
\includegraphics[width=1\textwidth,height=\textheight]{figures/fig2_relocation.pdf}
\caption{\textbf{Most raw-coordinate endpoint repairs relocate an error.} Bars partition the baseline failure set into full repair and error relocation after change supervision. Total height is endpoint repair; annotations report the relocated fraction among endpoint repairs. The three seeds share the 237-quartet bank and have baseline-failure denominators 110, 120, and 110. These are descriptive point estimates; per-seed parent-cluster intervals are given in the supplementary material.}
\end{figure}

\hypertarget{representation-and-supervision-can-yield-full-repair}{%
\subsection{Representation and supervision can yield full repair}\label{representation-and-supervision-can-yield-full-repair}}

The same protocol identifies a substantially more successful intervention. Table 1 shows the full input-by-supervision comparison. Geometric inputs alone increase \(J_3\) to 0.131--0.211. Combining them with change supervision increases it to \textbf{0.384--0.422}. For each seed, the combined arm improves over either component alone, and the corresponding paired parent-cluster intervals exclude zero.

\begin{longtable}[]{@{}llrrr@{}}
\toprule
Input & Supervision & Seed 11 & Seed 23 & Seed 47\tabularnewline
\midrule
\endhead
Coordinates & Clean & 0.055 & 0.051 & 0.055\tabularnewline
Coordinates & Change-augmented & 0.068 & 0.059 & 0.076\tabularnewline
Geometry & Clean & 0.131 & 0.169 & 0.211\tabularnewline
Geometry & Change-augmented & \textbf{0.384} & \textbf{0.422} & \textbf{0.397}\tabularnewline
\bottomrule
\end{longtable}

\textbf{Table 1. Joint correctness \(J_3\) on the same 237 quartets from 106 parents.} Hidden-layer design and task are shared across input choices. Training seeds are reported individually. ``Geometry'' denotes label-free indexed distances and centroid radii; it is not an oracle feature.
...
\hypertarget{the-protocol-also-recognizes-successful-visual-repair}{%
\section{The protocol also recognizes successful visual repair}\label{the-protocol-also-recognizes-successful-visual-repair}}

A useful diagnostic must distinguish improvement, not merely expose failure. We therefore apply the same accounting to a rendered geometric task. An ImageNet-initialized ResNet18 \citep{he2016deep, deng2009imagenet} receives RGB scenes. The evaluation bank contains 28 quartets from 19 parents. Training uses singleton states only; composed AB states are absent from the training set.

We compare a clean-only model with a model trained on the full singleton set. To separate extra supervision from the number of training presentations, the clean-only arm repeats 128 unique clean images to match the full arm's \textbf{323 presentations per epoch}. Both run for 20 epochs, select a checkpoint by loss on the same 81 singleton development images, and use the same seed-specific backbone initialization. The comparison matches exposure and selection, while intentionally changing the diversity and labels of supervised singleton states. It does not isolate data diversity from label coverage.

Figure 3 shows consistent gains. Clean-only joint correctness is 0.143, 0.250, and 0.214; full-singleton supervision reaches 0.571, 0.607, and 0.536. Paired differences are 0.429, 0.357, and 0.321, with 95\% parent-cluster intervals {[}0.222, 0.633{]}, {[}0.172, 0.556{]}, and {[}0.138, 0.517{]}. On the baseline 110 subsets, full repairs and relocations number \textbf{10 versus 1}, \textbf{9 versus 1}, and \textbf{5 versus 3}. The method is therefore credited with genuine constituent-preserving gains, while its remaining regressions stay visible.

\begin{figure}
\centering
\includegraphics[width=1\textwidth,height=\textheight]{figures/fig3_visual.pdf}
\caption{\textbf{Matched exposure reveals a genuine visual gain.} Left: joint correctness for clean-only and full-singleton supervision, with seeds shown individually. Right: paired differences and 95\% parent-cluster percentile intervals, using 10,000 draws. Both arms receive 323 presentations per epoch for 20 epochs and select on the same 81 singleton development images. All estimates use one 28-quartet, 19-parent bank.}
\end{figure}
...
\section{Related work and scope}\label{related-work-and-scope}}

\textbf{Compositional generalization.} Controlled recombination has long served as a test of systematic generalization \citep{lake2018generalization, keysers2020measuring}. Diagnostic visual reasoning datasets similarly use compositional structure to separate capabilities \citep{johnson2017clevr}. \citet{press2023measuring} explicitly study failures on a composed question despite correct answers to its subproblems. We build on that conditional logic. Our additional object is the \emph{paired change} in constituent and composed correctness when a model is repaired, under oracle-defined edits to one shared scene.

\textbf{Learning from semantic changes.} Counterfactually augmented data can expose distinctions that standard training underuses \citep{kaushik2020learning}. Excessive-invariance analysis emphasizes the complementary failure of ignoring task-relevant changes \citep{jacobsen2019excessive}. Our experiments concern these semantic changes rather than assuming that all small perturbations should preserve a prediction. The evidence does not establish that change supervision generally fails: its outcome depends on the representation and task.

\textbf{Preserving existing competence.} Positive-congruent training measures and discourages negative flips across model versions \citep{yan2021positive}. Error relocation is related, but its preservation set is coupled to each successful composed repair. The question is whether the repaired composition retains its own prerequisites. We claim neither to originate regression measurement nor to provide a new preservation objective.

\textbf{Scope.} Exact geometric labels make eligibility and outcome accounting unusually clear. They also limit external validity. Hand-designed geometric inputs encode useful task structure; a deterministic intersection parser can solve the coordinate problem. The representation comparison is consequently about the behavior of learned predictors under a controlled input change. The 237-quartet bank supports diagnostic comparisons, the fresh holdout supports a separately scoped failure observation, and the visual bank supports a small paired extension. None establishes a population-wide prevalence of relocation in large models. The accounting protocol applies more broadly wherever constituents and their composition can be labeled reliably; that broader empirical scope remains to be established.
