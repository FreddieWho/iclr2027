...
def train_mode(net, mode):
    """A frozen probe freezes BN buffers too; BN adaptation is a distinct arm."""
    net.train()
    if mode == "headonly":
        net.eval()
        net.fc.train()
...
def main():
    ...
    for mode in a.arms.split(","):
        ...
        opt = torch.optim.Adam(filter(lambda p: p.requires_grad, net.parameters()),
                               lr=3e-4)
        initial_backbone_hash = backbone_hash(net)
        batch_rng = torch.Generator().manual_seed(a.seed + 100_000)
        train_mode(net, mode.strip())
        ...
        with torch.no_grad():
            for qi, (q, m) in enumerate(QEV):
                ...
                logit = net(prep(imgs, a.res)).squeeze(-1).numpy()
                logits.append(logit)
                pl = (logit > 0).astype(int)
        ...
        res[mode.strip()] = {
            "backbone_hash_before": initial_backbone_hash,
            "backbone_hash_after": backbone_hash(net),
            "A_acc": float(okA.mean()), "B_acc": float(okB.mean()),
            "atomic_joint_acc": float((okA & okB).mean()),
        ...
        if mode.strip() == "headonly":
            assert initial_backbone_hash == backbone_hash(net), "Frozen backbone changed"
        np.savez_compressed(a.out / mode.strip() / "predictions.npz",
                            logits=np.array(logits), labels=L, parent_index=np.arange(len(L)))
