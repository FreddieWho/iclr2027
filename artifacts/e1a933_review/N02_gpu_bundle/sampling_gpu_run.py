...
import torchvision
import sampling_gpu_protocol as protocol
...
def main():
    ...
    try:
        ...
        if not torch.cuda.is_available():
            receipt['status']='NOT_RUN_NO_CUDA';receipt['unrun']=['4 distinct stem/input training-step preflights','native GPU image cache','48-arm training matrix'];save();return
        ...
        receipt['preflight']=probe_results;(out/'structure_matrix.json').write_text(json.dumps(structure,indent=2))
        ...
        assert len(receipt['completed'])==48;protocol.analyze(out,config['seeds'])
