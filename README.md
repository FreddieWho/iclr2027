...
P3 当前不需要新增数据域或更新 `infra/bioinf-data-index/`。先使用同一批 250 个 canonical samples、9 个冻结点集模型和 P2 的 provenance 链；正式运行前只执行 `P3-T0`，并将每个 T 任务的输入、manifest、checksum 和停止条件写入 `configs/phase3_support_geometry_v1.yaml`。
