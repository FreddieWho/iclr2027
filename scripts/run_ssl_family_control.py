...
def main() -> int:
    ...
    for seed in SEEDS:
        print(f"=== barlow_twins_2to1 seed={seed} ===", flush=True)
        T5R3.set_seed(seed)
    ...
    # intervention evaluation (frozen operator, eps=0.25, dev snapshots)
    ...
    for record in records:
        ...
        print(f"seed={record['seed']} intervention full rho={rho if rho is None else round(rho, 3)}", flush=True)
    ...
    summary = {"control_id": "barlow_twins_2to1", "role": "ssl_objective_family_control_NOT_selection",
                   ...
                   "n_intervention_sets": len(sets), "records": records}
