...
def run_one(seed: int, threads: int, output: Path) -> None:
    ...
    lock = verify_lock()
    T5R3 = load_module(ROOT / "scripts" / "run_t5r3_sanity.py", f"t5r3_for_v4_{seed}")
    M1BASE.T5R3_REF = T5R3
    R2 = load_module(ROOT / "scripts" / "run_t5r4_round2.py", f"r2_for_v4_{seed}")
    v4 = load_module(ROOT / "scripts" / "amr_model_v4.py", f"v4_for_m1v4_{seed}")
    sweep = load_module(ROOT / "scripts" / "run_p3_epsilon_sweep.py", f"sweep_for_v4_{seed}")
    sweep.EPSILONS = (0.25,)
    sweep.N_SNAPSHOTS = N_INTERVENTION_SNAPSHOTS
    T5R5 = load_module(ROOT / "scripts" / "run_t5r5_hidden_confirmation.py", f"t5r5_for_v4_{seed}")
    FC = load_module(ROOT / "scripts" / "p2_fracture_controls.py", f"fc_for_v4_{seed}")
...
def main() -> int:
    ...
    M1BASE = load_module(ROOT / "scripts" / "run_amr_m1.py", "m1base_for_v4")
