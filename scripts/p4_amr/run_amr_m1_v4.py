#!/usr/bin/env python3
"""P4-N4: AMR M1-redesign v4 training + dev evaluation (research routes 1+2).
...
Outputs: artifacts/phase4_amr/m1_v4/{record_seed*.json, models/,
...
def verify_lock() -> dict[str, Any]:
    lock = json.loads((P4 / "m1_v4_config_lock.json").read_text(encoding="utf-8"))
...
def run_one(seed: int, threads: int, output: Path) -> None:
    ...
    lock = verify_lock()
    T5R3 = load_module(ROOT / "scripts" / "p3_t5r" / "run_t5r3_sanity.py", f"t5r3_for_v4_{seed}")
    M1BASE.T5R3_REF = T5R3
    R2 = load_module(ROOT / "scripts" / "p3_t5r" / "run_t5r4_round2.py", f"r2_for_v4_{seed}")
    v4 = load_module(ROOT / "scripts" / "p4_amr" / "amr_model_v4.py", f"v4_for_m1v4_{seed}")
    sweep = load_module(ROOT / "scripts" / "p3_t5r" / "run_p3_epsilon_sweep.py", f"sweep_for_v4_{seed}")
    sweep.EPSILONS = (0.25,)
    sweep.N_SNAPSHOTS = N_INTERVENTION_SNAPSHOTS
    T5R5 = load_module(ROOT / "scripts" / "p3_t5r" / "run_t5r5_hidden_confirmation.py", f"t5r5_for_v4_{seed}")
    FC = load_module(ROOT / "scripts" / "p2" / "p2_fracture_controls.py", f"fc_for_v4_{seed}")
    ...
    print(f"=== AMR v4 seed={seed} threads={threads} ===", flush=True)
    T5R3.set_seed(seed)
    model = v4.AMRModelV4()
    ...
    ckpt = models_dir / f"amr_v4_seed{seed}.pt"
    ...
    record["lock_sha256_at_train"] = hashlib.sha256((P4 / "m1_v4_config_lock.json").read_bytes()).hexdigest()
...
def main() -> int:
    ...
    M1BASE = load_module(ROOT / "scripts" / "p4_amr" / "run_amr_m1.py", "m1base_for_v4")
