...
def main() -> int:
    ...
    m1 = load_module(ROOT / "scripts" / "p4_amr" / "run_amr_m1.py", "m1_for_cap")
    if args.merge:
    ...
    amr = load_module(ROOT / "scripts" / "p4_amr" / "amr_model.py", f"amr_for_cap_{args.seed}")
    T5R3 = load_module(ROOT / "scripts" / "p3_t5r" / "run_t5r3_sanity.py", f"t5r3_for_cap_{args.seed}")
    R2 = load_module(ROOT / "scripts" / "p3_t5r" / "run_t5r4_round2.py", f"r2_for_cap_{args.seed}")
    ...
    m1.verify_lock()
    m1.T5R3_REF = T5R3
    sweep = load_module(ROOT / "scripts" / "p3_t5r" / "run_p3_epsilon_sweep.py", f"sweep_for_cap_{args.seed}")
    sweep.EPSILONS = (0.25,)
    sweep.N_SNAPSHOTS = m1.N_INTERVENTION_SNAPSHOTS
    T5R5 = load_module(ROOT / "scripts" / "p3_t5r" / "run_t5r5_hidden_confirmation.py", f"t5r5_for_cap_{args.seed}")
    FC = load_module(ROOT / "scripts" / "p2" / "p2_fracture_controls.py", f"fc_for_cap_{args.seed}")
