...
def run_one(seed: int, threads: int, output: Path) -> None:
    ...
    lock = verify_lock()
    T5R3 = load_module(ROOT / "scripts" / "p3_t5r" / "run_t5r3_sanity.py", f"t5r3_for_v5cap_{seed}")
    M1BASE.T5R3_REF = T5R3
    R2 = load_module(ROOT / "scripts" / "p3_t5r" / "run_t5r4_round2.py", f"r2_for_v5cap_{seed}")
    v4 = load_module(ROOT / "scripts" / "p4_amr" / "amr_model_v5cap.py", f"v5cap_for_m1v5cap_{seed}")
    sweep = load_module(ROOT / "scripts" / "p3_t5r" / "run_p3_epsilon_sweep.py", f"sweep_for_v5cap_{seed}")
    sweep.EPSILONS = (0.25,)
    sweep.N_SNAPSHOTS = N_INTERVENTION_SNAPSHOTS
    T5R5 = load_module(ROOT / "scripts" / "p3_t5r" / "run_t5r5_hidden_confirmation.py", f"t5r5_for_v5cap_{seed}")
    FC = load_module(ROOT / "scripts" / "p2" / "p2_fracture_controls.py", f"fc_for_v5cap_{seed}")
...
def main() -> int:
    ...
    M1BASE = load_module(ROOT / "scripts" / "p4_amr" / "run_amr_m1.py", "m1base_for_v5cap")
