...
Route 3 (GradNorm controller) NOT implemented; instead init-calibrated
  fixed W_ROUTE (trunk grad-norm parity at init, frozen in lock).
...
Outputs: artifacts/phase4_amr/m1_v5/{record_seed*.json, models/,
...
W_ROUTE = 2.0  # kept from v4b (warm-start + ramp schedule below; isolates architecture)
...
def verify_lock() -> dict[str, Any]:
    lock = json.loads((P4 / "m1_v5_config_lock.json").read_text(encoding="utf-8"))
...
def route_batch(model: Any, T5R3: Any, v4: Any, batch: dict[str, torch.Tensor],
    ...
    """One route update: Slepian intervention, stop-grad clean target,
    normalized direction recovery on the sampled band. Returns losses +
    diagnostics (mu, band, support size, inv response measured not optimized)."""
    ...
    for i in range(n):
        ...
        deltas.append(delta)
        mus.append(mu)
        evecs = spec_cache["U"][snap_idx[i]]
        ev = spec_cache["evals"][snap_idx[i]]
        lo, hi = v4.BAND_EDGES[band], v4.BAND_EDGES[band + 1]
        m = (ev >= lo) & (ev < hi if band < 2 else ev <= hi)
        ub = evecs[:, m]
        proj = ub @ ub.T if m.any() else np.zeros((20, 20))
        targets.append((proj @ delta).reshape(-1))
    delta_t = torch.from_numpy(np.stack(deltas)).float()
    target_pooled = T5R3.team_pool(torch.from_numpy(np.stack(targets)).float()
                                   .reshape(n, 20, 2), team)
    feats_before = model.band_features(centered, team, adj,
                                       (torch.from_numpy(spec_cache["evals"][snap_idx]),
                                        torch.from_numpy(spec_cache["U"][snap_idx]))).detach()
    feats_after = model.band_features(centered + delta_t, team, adj,
                                      (torch.from_numpy(spec_cache["evals"][snap_idx]),
                                       torch.from_numpy(spec_cache["U"][snap_idx])))
    d_r = feats_after[:, band, :] - feats_before[:, band, :]
    pred = model.eqv_heads[band](d_r)
    l_eqv = ((F.normalize(pred, dim=-1) - F.normalize(target_pooled, dim=-1)).pow(2)
             .sum(dim=-1).mean())
    inv_response = d_r.pow(2).sum(dim=-1).mean().detach()
    return {"loss": l_eqv, "l_eqv": l_eqv.detach(), "inv_response": inv_response,
...
def run_one(seed: int, threads: int, output: Path) -> None:
    ...
    lock = verify_lock()
    T5R3 = load_module(ROOT / "scripts" / "p3_t5r" / "run_t5r3_sanity.py", f"t5r3_for_v5_{seed}")
    M1BASE.T5R3_REF = T5R3
    R2 = load_module(ROOT / "scripts" / "p3_t5r" / "run_t5r4_round2.py", f"r2_for_v5_{seed}")
    v4 = load_module(ROOT / "scripts" / "p4_amr" / "amr_model_v5.py", f"v5_for_m1v5_{seed}")
    sweep = load_module(ROOT / "scripts" / "p3_t5r" / "run_p3_epsilon_sweep.py", f"sweep_for_v5_{seed}")
    sweep.EPSILONS = (0.25,)
    sweep.N_SNAPSHOTS = N_INTERVENTION_SNAPSHOTS
    T5R5 = load_module(ROOT / "scripts" / "p3_t5r" / "run_t5r5_hidden_confirmation.py", f"t5r5_for_v5_{seed}")
    FC = load_module(ROOT / "scripts" / "p2" / "p2_fracture_controls.py", f"fc_for_v5_{seed}")
    ...
    print(f"=== AMR v5 seed={seed} threads={threads} ===", flush=True)
    T5R3.set_seed(seed)
    model = v4.AMRModelV5()
    ...
    record["lock_sha256_at_train"] = hashlib.sha256((P4 / "m1_v5_config_lock.json").read_bytes()).hexdigest()
...
def merge(output: Path) -> int:
    ...
    summary = {"method": "amr_v5_decoupled",
               "lock": "artifacts/phase4_amr/m1_v5_config_lock.json",
               "evidence_scope": "train/valid dev only", "n_intervention_sets": records[0]["n_intervention_sets"],
               "records": records}
    write_json(output / "summary.json", summary)
    manifest = {"status": "P4_R1_AMR_V5_DEV_COMPLETE",
                "seeds": list(SEEDS),
...
def main() -> int:
    ...
    M1BASE = load_module(ROOT / "scripts" / "p4_amr" / "run_amr_m1.py", "m1base_for_v5")
