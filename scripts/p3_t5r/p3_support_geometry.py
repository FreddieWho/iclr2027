...
def load_frozen_model(root: Path, model_id: str) -> FrozenGeometryModel:
    adapter_path = root / "scripts" / "p2" / "p2_point_model_adapter.py"
...
def run_prospective_generate(root: Path, config_path: Path, config: dict[str, Any]) -> dict[str, Any]:
    ...
    manifest = {
        ...
        "source_p2_generator": output_record(root, root / "scripts" / "p2" / "run_phase2_fracture_formal.py"),
        "runtime": {
            "wall_seconds": time.perf_counter() - started,
            "peak_rss_mib": resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1024,
        },
    ...
    write_json(temporary_dir / "source_input_provenance.json", {
        ...
        "p2_generator": output_record(root, root / "scripts" / "p2" / "run_phase2_fracture_formal.py"),
    })
