...
状态：已执行（2026-09-24）。用在 L-015 的四条主臂上，各 10 个种子，评价是 dev512 的 237/106，不是封存确认池。
G8 排序特征对 raw 的差为 +0.435 至 +0.532（种子标准差 0.033，10/10 父场景区间排除 0，最弱下界 +0.316），远高于 ±15pp 地板。
置换增广对 raw 为 +0.144 至 +0.270（种子标准差 0.048，10/10 排除 0），均值过 12pp 门槛，但最弱区间下界只有 +0.044，没有特征臂那么远。
可学习段集合没有正向差。见 `reports/e1a933_review/L015_L006_REPORT.md`。
...
状态：已执行（2026-09-24）。复读 `artifacts/event_updater/sided_s{11,23,47}.json`，未重跑、未发展 loss。
miss 组 R 中位 1.896/1.870/1.871，hit 组 0.638/0.808/0.935；oracle 半径中位约 0.18 vs 0.20。
可引用句写在 `reports/e1a933_review/L014_RADIUS_REPORT.md`。正文第 9 页已满，本轮不把这句观察挤进讨论段。
...
状态：已执行（2026-09-24）。训练版本已跑，未升为方法主张，也未读封存确认池。
置换增广抬高 J（均值 +0.195）但不去掉标号依赖（绝对摆动不降）。
G8 排序特征同时去掉摆动（logit 差 0）并抬高 J（均值 +0.472）。
可学习的段集合在同一 300 轮配方下学不会（宽 32 与匹配参数量的宽 48 都是 J 约 0.02）。
见 `reports/e1a933_review/L015_L006_REPORT.md` 与附录 `paper/sections/app_l015.tex`。
