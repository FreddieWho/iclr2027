...
Fixed lambda_aux=0.5 (loss-scale matching, no search). Seeds 803/805/806.
...
def main():
    ...
    R10 = (rel10(C) - rel10(C).mean(0)) / (rel10(C).std(0) + 1e-8)
    ...
    if "Cshuf" in a.arms.split(","):
        sh = torch.randperm(len(T10))
        T10S = T10[sh]
    else:
        T10S = None
    ...
    for mode in [m.strip() for m in a.arms.split(",")]:
        ...
        aux_dim = {"A": 0, "B": 8, "C": 10, "Cshuf": 10}[mode]
        net = DistillNet(aux_dim)
        opt = torch.optim.Adam(net.parameters(), lr=3e-4)
        T = {"A": None, "B": T08, "C": T10, "Cshuf": T10S}[mode]
        net.train()
        for ep in range(a.epochs):
            ...
            for i in range(0, len(Xb), 128):
                ...
                opt.zero_grad()
                if T is None:
                    loss = bce(net(Xb[b]), yb[b])
                else:
                    pe, pa = net(Xb[b])
                    loss = bce(pe, yb[b]) + LAM_AUX * mse(pa, T[b])
