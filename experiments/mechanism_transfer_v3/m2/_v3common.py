"""Deterministic import of the v3 shared package from inside ``m2``.

The repository has other modules named ``common`` (for example
``experiments/discovery_campaign/common.py``) and several legacy modules insert
their own directories on ``sys.path``. Resolving the v3 package by file
location keeps the M2 code independent of ``sys.path`` order, so an M2 run can
never silently consume a legacy helper instead of the frozen v3 contracts.
"""
