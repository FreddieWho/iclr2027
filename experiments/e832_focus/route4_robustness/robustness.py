...
def transform(x,kind):
 ...
 if kind=='noise':
  # fixed per-parent noise, same perturbation within all four E states
  out=np.empty_like(x)
  for j,p in enumerate(np.unique(x[:,0,0])):
   # x is [states,n,4,2] in caller; this branch is not used directly
   pass
  return out
 raise KeyError(kind)
def main():
 ...
 for arm in ('raw','additive','repaired','rich_unsorted','rich_sorted'):
  for seed in (11,23,47):
   ...
   for cond in conditions:
    st=B['states'].copy()
    if cond=='translation':st=st+np.array([.071,-.043])
    elif cond=='rotation':
     t=np.deg2rad(7);R=np.array([[np.cos(t),-np.sin(t)],[np.sin(t),np.cos(t)]]);st=st@R.T
