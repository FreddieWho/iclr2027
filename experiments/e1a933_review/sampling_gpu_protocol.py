...
def net_for(init,stem,seed,weight_file):
...
def make_arms():
    ...
    for observation in ['C','D']:
        for init in ['pretrained','random']:
            arms.append(dict(observation=observation,stem='standard',init=init,supervision='flip'))
    for observation in ['A','B']:
        for init in ['pretrained','random']:
            arms.append(dict(observation=observation,stem='standard',init=init,supervision='static'))
    return arms
...
def input_batch(maps,split,ix,observation,device):
    source='A' if observation in ['A','B'] else 'C'
...
def train_arm(data,maps,out,arm,seed,weight_file,epochs=20,batch=32,device='cuda'):
    ...
    net=net_for(arm['init'],arm['stem'],seed,weight_file).to(device)
