#!/usr/bin/env python3
"""P3-D FAIR rerun (§12: identical base coeff + lam; keepbal = base + lam*(0.5 flip + 0.5 preserve)).
...
OUT = ROOT / "artifacts" / "next_novelty" / "p3d_fair"
...
def main():
    ...
    lam = 1.0  # identical across arms (fair rerun §12)
    arms = {
        "fliprep": [("base", Xc, yc, 1.0), ("aug", Xf, yf, lam)],
        "fliphalf": [("base", Xc, yc, 1.0), ("aug", Xfh, yfh, lam)],
        "keepbal": [("base", Xc, yc, 1.0),
                    ("augh", Xfh, yfh, lam * 0.5), ("augp", Xph, yph, lam * 0.5)],
    }
    ...
    for name, terms in arms.items():
        ...
        for ep in range(a.epochs):
            model.train()
            opt.zero_grad()
            loss = sum(w * bce(model(Xt), yt) for _, Xt, yt, w in terms)
            loss.backward()
            opt.step()
        model.eval()
